			...
			if (true) { 				
				...
				if ($valid_txnid && $data['payment_status'] == "Completed") {
					...
					$enddate_1=date('Y-m-d', strtotime($enddate. ' -'.(int)$d.' Days')); //added on 19-07-2016
						...
						try {
							$stmt1 = $dbcon->prepare($sql1, array(PDO::ATTR_CURSOR => PDO::CURSOR_SCROLL));
							$stmt1->execute(array($agent_id,$sub_id,$boq_id,$plan_id,$data['payment_amount'],$startdate,$enddate_1,$created_datetime1));	
							$stmt1 = null;
						}catch (PDOException $e){
							print $e->getMessage();
						}
						...
						Subscription Amount: ".$data['payment_amount']." <br><br>
