...
if ( !class_exists( 'PT_CV_Functions' ) ) {
	...
	class PT_CV_Functions {
		...
		static function cv_strip_shortcodes( $text ) {
			if ( apply_filters( PT_CV_PREFIX_ . 'ignore_shortcode_content', false ) ) {
		...
		static function cv_strip_tags( $string ) {
			...
			if ( apply_filters( PT_CV_PREFIX_ . 'strip_tags', 1 ) ) {
				...
				$string = preg_replace( apply_filters( PT_CV_PREFIX_ . 'excerpt_clean_tags', array( '@<(script|style)[^>]*?>.*?</\\1>@si' ) ), '', $string );
