	...
	<div id="pmpro_account">		
		...
		<?php if(in_array('membership', $sections) || in_array('memberships', $sections)) { ?>
			<div id="pmpro_account-membership" class="pmpro_box">
				...
				<h3><?php _e("My Subscriptions", "pmpro");?></h3> <!-- added -->
				<table width="100%" cellpadding="0" cellspacing="0" border="0">
					<thead>
						<tr>
							<!-- <th><?php _e("Level", "pmpro");?></th> -->
							<th><?php _e("Plan", "pmpro");?></th> <!-- added -->
							<th><?php _e("Billing", "pmpro"); ?></th>
							<!-- <th><?php _e("Paid On", "pmpro"); ?></th> -->	<!-- added -->
							<th><?php _e("Starts From", "pmpro"); ?></th> <!-- added -->
							<th><?php _e("Expires On", "pmpro"); ?></th>  <!-- added -->
						...
						<tr>
							<td class="pmpro_account-membership-levelname">
								...
								<div class="pmpro_actionlinks">
									...
									<?php } ?>
									<a id="plancancelLink" href="<?php echo pmpro_url("cancel", "?level=" . $current_user->membership_level->id)?>"><?php _e("Cancel<a>when<?php echo "hi"; ?></a><?php echo "hi"; echo do_shortcode( '[hidecancel]' ); ?>				", "pmpro");?> </a>
							...
							<!-- added -->
							<!-- <td class="pmpro_account-membership-expiration pmpro_billpaid">
							<?php 
								if($current_user->membership_level->startdate) 
									{echo date(get_option('date_format'),current_time('mysql'));
									}
							...
							<td class="pmpro_account-membership-expiration pmpro_start">
							<?php 
								if($current_user->membership_level->startdate) 
									{//$date=date(get_option('date_format'), $current_user->membership_level->startdate); 
									//$date=new DateTime(date(get_option('date_format'), $current_user->membership_level->startdate));
								//$date->add(new DateInterval('P30D'));
									//date_add($date, date_interval_create_from_date_string('30 days'));
									//echo date(get_option('date_format'), $date);
									//echo $date->format('F d, Y');
									echo date(get_option('date_format'), $current_user->membership_level->startdate);}
				...
				<div class="pmpro_actionlinks">
					...
					<a href="<?php echo pmpro_url("levels")?>"><?php _e("View all Subscription Options", "pmpro");?></a> <!-- added -->
				</div>
	 ...
	 <?php if(in_array('profile', $sections)) { ?>
			<div id="pmpro_account-profile" class="pmpro_box" style="display:none;">	
			...
			<table width="100%" cellpadding="0" cellspacing="0" border="0">
				<thead>
					<tr>
						...
						<!-- <th><?php _e("Level", "pmpro"); ?></th> --><!-- added -->
						<th><?php _e("Plan", "pmpro"); ?></th> <!-- added -->
