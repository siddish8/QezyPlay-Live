...
//$enddate = $wpdb->get_var("SELECT UNIX_TIMESTAMP(enddate) as enddate FROM $wpdb->pmpro_memberships_users WHERE user_id = '" . $user->ID . "' AND membership_id = '" . $old_level_id . "' AND status IN('active', 'admin_cancelled') ORDER BY id DESC"); 	
...
$plan_id=$wpdb->get_var("SELECT membership_id FROM $wpdb->pmpro_memberships_users WHERE user_id = '" . $user->ID . "' AND status IN('active', 'admin_cancelled') ORDER BY id DESC"); 



$enddate=$wpdb->get_var("select enddate from wp_pmpro_memberships_users where user_id=".$user->ID." and status='active' order by id desc limit 1");
...
$plan=$wpdb->get_var("SELECT name FROM $wpdb->pmpro_membership_levels WHERE id = '" . $plan_id . "' LIMIT 1");		
...
$this->data['membership_level_name'] = $wpdb->get_var("SELECT name FROM $wpdb->pmpro_membership_levels WHERE id = '" . $old_level_id . "' LIMIT 1");
			...
			$this->subject = sprintf(__("Your subscription at %s has been CANCELLED", "pmpro"), get_option("blogname"));			

			$this->template = "cancel";

			$this->data = array("name" => $user->display_name, "user_login" => $user->user_login, "membership_level_name"=>$plan,"enddate"=>$enddate,"sitename" => get_option("blogname"), "siteemail" => pmpro_getOption("from_email"),);
...

		{
			...
			$this->subject = sprintf(__("Subscription for %s at %s has been CANCELLED", "pmpro"), $user->user_login, get_option("blogname"));			

			$this->template = "cancel_admin";

			$this->data = array("user_login" => $user->user_login, "user_email" => $user->user_email, "display_name" => $user->display_name, "sitename" => get_option("blogname"), "siteemail" => pmpro_getOption("from_email"), "login_link" => site_url()."/login/");
...
$plan=$wpdb->get_var("SELECT name FROM $wpdb->pmpro_membership_levels WHERE id = '" . $plan_id . "' LIMIT 1");		
...

		{
			...
			//do_action('update_delay');

			//do_action('sub_sup');



			//$plan_startdate=$wpdb->get_var("select plan_start_date from pmpro_dates_chk1 where user_id=".$current_user->ID." order by id desc limit 1 ");

/* added */



$plan_startdate=$wpdb->get_var("select plan_startdate from pmpro_dates_chk1 where user_id=".$user->ID." order by id desc limit 1 ");	

$plan_startdate=new DateTime($plan_startdate);

$plan_startdate=$plan_startdate->format("d-m-Y");

$next_paydate=$wpdb->get_var("select next_paydate from pmpro_dates_chk1 where user_id=".$user->ID." order by id desc limit 1 ");	

//$delay=$wpdb->get_var("select delay from pmpro_dates_chk1 where user_id=".$user->ID." order by id desc limit 1 ");	
$delay=get_option("pmpro_sub_support_prev_delay_".$user->ID);
$old_pid=$wpdb->get_var("select old_plan_id from pmpro_dates_chk1 where user_id=".$user->ID." order by id desc limit 1 ");	

$old_pname=$wpdb->get_var("select old_plan_name from pmpro_dates_chk1 where user_id=".$user->ID." order by id desc limit 1 ");	
			...
			$this->subject = sprintf(__("Your subscription confirmation for %s", "pmpro"), get_option("blogname"));	
...
global $wpdb, $current_user;
			...
			if($enddate)
				...
				$this->data["membership_expiration"] = "<p>" . sprintf(__("This subscription will expire on %s.", "pmpro"), date_i18n(get_option('date_format'), $enddate)) . "</p>\n";
...
//$startdate = $wpdb->get_var("SELECT UNIX_TIMESTAMP(startdate) FROM $wpdb->pmpro_memberships_users WHERE user_id = '" . $user->ID . "' AND status = 'active' LIMIT 1");

$startdate = $wpdb->get_var("SELECT startdate FROM wp_pmpro_memberships_users WHERE user_id = '" . $user->ID . "' AND status = 'active' LIMIT 1");
...
$old_pname=$wpdb->get_var("select old_plan_name from pmpro_dates_chk1 where user_id=".$user->ID." order by id desc limit 1 ");	
...

		{
			...
			$this->subject = sprintf(__("Subscription Payment Failed at %s", "pmpro"), get_option("blogname"));	
		...
		{		
			...
			$this->subject = sprintf(__("Subscription Payment Failed For %s at %s", "pmpro"), $user->display_name, get_option("blogname"));	
...
$plan_startdate=$wpdb->get_var("select plan_startdate from pmpro_dates_chk1 where user_id=".$user->ID." order by id desc limit 1 ");	
			...
			$this->subject = sprintf(__("INVOICE for %s subscription", "pmpro"), get_option("blogname"));	
...

		{
			...
			$this->subject = sprintf(__("Your subscription at %s has ended", "pmpro"), get_option("blogname"));			

			$this->template = "membership_expired";

			$this->data = array("subject" => $this->subject, "name" => $user->display_name, "user_login" => $user->user_login, "sitename" => get_option("blogname"), "siteemail" => pmpro_getOption("from_email"), "login_link" => site_url()."/login/", "display_name" => $user->display_name, "user_email" => $user->user_email, "levels_link" => pmpro_url("levels"));			
...

		{
			...
			$this->subject = sprintf(__("Your subscription at %s has been changed", "pmpro"), get_option("blogname"));

			$this->template = "admin_change";

			$this->data = array("subject" => $this->subject, "name" => $user->display_name, "user_login" => $user->user_login, "sitename" => get_option("blogname"), "membership_id" => $user->membership_level->id, "membership_level_name" => $user->membership_level->name, "siteemail" => pmpro_getOption("from_email"), "login_link" => site_url()."/login/");

			if($user->membership_level->ID)

				$this->data["membership_change"] = sprintf(__("The new plan is %s", "pmpro"), $user->membership_level->name);

			else

				$this->data["membership_change"] = __("Your subscription has been cancelled", "pmpro");

			if(!empty($user->membership_level->enddate))

			{

					$this->data["membership_change"] .= ". " . sprintf(__("This subscription will expire on %s", "pmpro"), date_i18n(get_option('date_format'), $user->membership_level->enddate));
...

			{
				...
				$this->data["membership_change"] .= ". " . __("This subscription does not expire", "pmpro");
...

		{
			...
			$this->subject = sprintf(__("Subscription for %s at %s has been changed", "pmpro"), $user->user_login, get_option("blogname"));

			$this->template = "admin_change_admin";

			$this->data = array("subject" => $this->subject, "name" => $user->display_name, "user_login" => $user->user_login, "sitename" => get_option("blogname"), "membership_level_name" => $user->membership_level->name, "siteemail" => get_bloginfo("admin_email"), "login_link" => site_url()."/login/");
			...
			if($user->membership_level->ID)
				...
				$this->data["membership_change"] = __("Subscription has been cancelled", "pmpro");
