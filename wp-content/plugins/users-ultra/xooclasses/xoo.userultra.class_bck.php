...
[usersultra_directory list_per_page=8 optional_fields_to_display='friend,social,country,description' pic_boder_type='rounded']",
	...
	public function create_register_page() 
	{
		if (!$this->get_option('registration_page_id')) {
			...
			$new = array(
			  'post_title'    => __('Register','xoousers'),
			  'post_type'     => 'page',
			  'post_name'     => $slug,
...
$date_picker_array = array(
	...
	public function password_reset( $args=array() ) {
		...
		$display .= '<div class="xoouserultra-head" style="background:none;padding:0px 20px">';
		    $display .='<div class="xoouserultra-left">';
		        $display .='<div style="margin: 15px auto;" class="xoouserultra-field-name xoouserultra-field-name-wide login-heading" id="login-heading-'.$this->login_code_count.'">'.__('Reset Password','xoousers').'</div>';
		    $display .='</div>';
    ...
    <label id="'.$meta2.'_label" class="xoouserultra-field-type" style="display:none">Error</label><div id="'.$meta.'_err" class="alert alert-danger xoouserultra-field-value"></div>
   </div>';		
		...
		$display .= '</div><div id="'.$meta_2.'_err_div" class="xoouserultra-field" style="display:none">
    <label id="'.$meta_2.'_label" class="xoouserultra-field-type" style="display:none">Error</label><div id="'.$meta_2.'_err" class="alert alert-danger xoouserultra-field-value"></div>
   </div>';		
		$display .= '</div><div class="xoouserultra-clear"></div>';
		...
		$display .= '<div class="xoouserultra-field xoouserultra-edit xoouserultra-edit-show">';
		$display .= '
    <label id="btn_rst_label" class="xoouserultra-field-type" style="display:none">B</label><div id="btn_rst_div" class="xoouserultra-field-value">';
		$display .= '<input style="margin: 20px auto;"type="submit" onclick="return validate(this.id)" name="xoouserultra-login" class="xoouserultra-button xoouserultra-reset-confirm'.$login_btn_class.'" value="'.__('Reset Password','xoousers').'" id="xoouserultra-reset-confirm-pass-btn" />';
		$display .= '</div></div><div class="xoouserultra-clear"></div>';
		$display .= '<br>';	
...
$(".blog-heading").css({"padding":"0px"})
setTimeout(function(){

//document.getElementById("reset-pwd-success").style.display="none";

},3000);
$("#preset_password").click(function() {
 $("#reset-pwd-success").slideUp();
});

$("#preset_password_2").click(function() {
 $("#reset-pwd-success").slideUp();
});

$("#xoouserultra-reset-confirm-pass-btn").click(function() {
 $("#reset-pwd-success").slideUp();
});
	$("#preset_password").click(function() {
...
$display .= '<i class="fa fa-'.$icon.'"></i>';
 $display .= ''.$name. ' '.$required_text.'</label>';
    ...
    <label id="'.$meta.'_label" class="xoouserultra-field-type" style="display:none">Error</label><div id="'.$meta.'_err" class="alert alert-danger xoouserultra-field-value"></div>
   </div>';
		...
		$forgot_pass = '<div align="center"><a style="color: #FF5722;font-weight:bolder;" href="../forgot-password/" id="xoouserultra-forgot-pass-'.$this->login_code_count.'" class="xoouserultra-login-forgot-link" title="'.__('Forgot Password?','xoousers').'">'.__('Forgot Password?','xoousers').'</a></div>';
		...
		{			
			$register_link_url = '<a style="background-color:#3994b7;color:white;border:1px solid #3994b7;font-weight:bolder;" href="'.$register_link.'" class="btn btn-default">'.__('New User Registration','xoousers').'</a>'; //xoouserultra-login-register-link |
		}
		...
		<div><p style="margin:0;font-size:12px;float:right;" id="tos_pp">*By clicking on Login, you agree to our <a href="../terms-of-service">Terms of Service</a> and that you have read our <a href="../privacy-policy">Privacy Policy</a></p> </div>
		<input onclick="return validate(this.id)" type="submit" id="Login"  name="xoouserultra-login" class="btn btn-success'.$login_btn_class.'" value="'.__('Login','xoousers').'" />'.$forgot_pass.$register_link_url;
...
$forgot_pass = '';
		...
		$forgot_pass .= '<style>label.xoouserultra-field-type {
    margin-top: 10px;
}</style><div class="xoouserultra-inner xoouserultra-login-wrapper" style="padding: 10px;
    max-width: 500px;
    margin: 0 auto;
...
$forgot_pass.='<div class="xoouserultra-back-to-login">';
     ...
     <div id="user_name_email_err" class="alert alert-danger xoouserultra-field-value"></div>
   </div>';
...
$(".blog-heading h1").hide();
$(".blog-heading").css({"padding":"0px"})
$("#user_name_email").click(function() {
 $("#uultra-signin-ajax-noti-box").slideUp();
});
...
return $forgot_pass;
	...
	function show_registration_form( $args=array() )
	{
		...
		$defaults = array(
        'use_in_sidebar' => null,
		'form_header_text' => __('Register for 1 Week Free-Trial','xoousers'),
		'custom_text' => '',
        'redirect_to' => null
...
//style="background: url('.site_url().'/wp-content/uploads/2016/09/bck2.jpg)"
	...
	function display_the_registeration_form( $sidebar_class=null, $redirect_to=null , $args)
	{
		...
		//UserName should not contain special char and shouldn't start with underscore and must be minm. of length 4
								if($meta=="user_login"){$help="User name must be between 4 - 10 alphanumeric characters(Aa-Zz 0..9) and should not start with underscore";}
...
//Password must be of 4 minimum length
    ...
    <label id="'.$meta.'_label" class="xoouserultra-field-type"></label><div id="'.$meta.'_err" class="alert alert-danger xoouserultra-field-value"></div>
		...
		$display .='<div class="xoouserultra-field xoouserultra-edit xoouserultra-edit-show">Got a Coupon? <a href="javascript:void(0)" onclick="return cc_form()">Click Here</a><div id="cc_div_show" style="display:none"></div></div>';
		...
		$display .='<script>
				...
				if(ok=="none") {
					...
					jQuery(\'#cc_div_show\').html(\'<label class="xoouserultra-field-type" for="cc"><i class="fa fa-tags" aria-hidden="true"></i><span>Enter Coupon Code </span></label><div class="xoouserultra-field-value"><input onkeypress="return event.charCode !=32" onblur="return validate(this.id)" onclick="return styleback(this.id)" type="text" class="xoouserultra-input" name="cc" id="cc" value="" title=""><div id="cc_err_div" class="xoouserultra-field" style="display:none"><label id="cc_label" class="xoouserultra-field-type"></label><div id="cc_err" class="alert alert-danger xoouserultra-field-value"></div> </div><a href="javascript:void(0)" onclick="return cc_div_hide()">No Coupon</a></div><div class="xoouserultra-help"></div><div class="xoouserultra-clear"></div><br>\');
...
function avoidspace(id)
{
	...
	public function get_social_buttons ($action_text, $atts)
	{
		...
		if($this->get_option('registration_rules')!=4) // Social media is not able when using paid registrations
		{
			...
			$atleast_one = false;
			//<br />
			$divider .='<div class="xoouserultra-or-divider">	<div>OR</div>	</div>';
			$divider .='<div align="center" style="color:#888;font-size:12px;margin:0 auto;"> *By Sign-in or Sign-up with Facebook, 				you agree to our <a href="../terms-of-service">Terms of Service</a> and that you have read our <a 				href="../privacy-policy">Privacy Policy</a></div>';
			...
			if($this->get_option('social_media_fb_active')==1)
			{
				...
				$display .=$divider.'<div class="txt-center FacebookSignIn">
						...
						<a style="text-decoration:none" href="'.$loginUrl.'" class="btnuultra-facebook" >
