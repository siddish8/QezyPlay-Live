...
class XooShortCode {
	...
	*/
	...
	{	
		...
		add_shortcode('usersultra_forgot_pswd',array(&$this,'usersultra_forgot_pswd_fn'))	;	//added	
	...
	*/
	...
	public function usersultra_forgot_pswd_fn() //added
	{
		global $xoouserultra;
		

		return $xoouserultra->forgot_password_form();
	}
		...
		if (isset($_GET['resskey'])&& $_GET['resskey']!="") 
		{
			...
			if (!is_user_logged_in()) 
			{
				...
				if($xoouserultra->get_option('uultra_auto_redirect_loggedin_user') == 'yes')
				{
					...
					//return $xoouserultra->show_minified_profile( $atts );	//added/changed
