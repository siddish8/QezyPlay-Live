...
//added
function new_user_regsub($user_id,$startdate,$plan_id)
{
		global $wpdb;
		$txt="---------------- /n ";
		...
		$cycle_period_no=$wpdb->get_var("SELECT expiration_number FROM wp_pmpro_membership_levels where id=".$planid." ");
		$cycle_period=$wpdb->get_var("SELECT expiration_period FROM wp_pmpro_membership_levels where id=".$planid." ");
...
//ended
				...
				//else{
					...
					fwrite($file,$info1);	
						if($cpn=="1" or $cpn=="2")
							{
							if($cpn=="1")
							$this->errors[] = __('<strong>ERROR:</strong> Please enter a valid coupon code.','xoousers'); 					
							if($cpn=="2")
							$this->errors[] = __('<strong>ERROR:</strong> Coupoon code already used','xoousers'); 					
		...
		session_start();
			...
			if (!isset($this->errors)) 
			{
				...
				if ( ! $user_id ) 
				{
					...
					$this->new_user_regsub($user_id,$startdate,$plan_id);
	...
	function get_errors() {
		...
		$display .= '<div class="xoouserultra"><span><i class="fa fa-ok"></i><script>swal("Thank you for Registering with Qezyplay.Please activate your account from registered email.")</script></span></div>';
