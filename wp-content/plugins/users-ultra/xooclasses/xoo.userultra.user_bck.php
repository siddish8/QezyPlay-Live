	...
	public function confirm_reset_password()
	{
		...
		if($validation=="" )
		{
			...
			if($user->ID >0 )
			{
				...
				$html = "<div id='reset-pwd-success' class='uupublic-ultra-success'>".__(" Success!! The new password has been sent to ".$user_email."  ", 'xoousers')."</div>";
				...
				$html .= "<script>document.getElementById('preset_password').value='';document.getElementById('preset_password_2').value='';</script>";
...
if ( $type == 'usermeta' && $deleted == 0 && $private == 0  && isset($array[$key]['show_in_register']) && $can_hide ==0)	
			{				
				...
				if (isset($array[$key]['name']) && $name)
				{
					if($name=="Profile picture"); elseif($this->get_custom_user_meta( $meta, $user_id) != "") $html .= ' <span class="data-a">'.$name.'</span><span class="data-b"><strong>'.$this->get_custom_user_meta( $meta, $user_id).'</strong></span> ';
				}
...
public function  get_profile_info2 ($user_id)	
	{
		...
		foreach($array as $key => $field) 
		{
			...
			{				
				...
				if (isset($array[$key]['name']) && $name)
				{
					if($meta=="user_pic"); else if($this->get_custom_user_meta( $meta, $user_id) != "") $html .= ' <span class="data-a">'.$name.'</span><span class="data-b"><strong>'.$this->get_custom_user_meta( $meta, $user_id).'</strong></span> ';
				}
...
public function  get_profile_info ($user_id)	
	{
		...
		foreach($array as $key => $field) 
		{
			...
			{				
				...
				if (isset($array[$key]['name']) && $name)
				{
					if($meta=="user_pic"); else if($this->get_custom_user_meta( $meta, $user_id) != "") $html .= ' <span class="data-a">'.$name.':</span><span class="data-b">'.$this->get_custom_user_meta( $meta, $user_id).'</span> ';
				}
