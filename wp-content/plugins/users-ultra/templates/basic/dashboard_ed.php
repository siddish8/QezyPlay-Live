...
if($pass!='')
{
	if ( $current_user && wp_check_password( $pass, $user_pwd, $user_id) )   
		...
		echo "<style>#view{display:block !important;}.pwd{display:none !important;}</style>"; 
			...
			echo '<script>document.getElementById("PS").setAttribute("class","tablinks active");
...
?></form>
			...
			<div id="view" class="commons-panel-content" style="display:none">
                       ...
                       <p class="f15"><?php  _e('Type your New Password','xoousers');?></p>
                 			 <p><input onclick="stylebackE(this.id)" onkeypress="return event.charCode !=32" type="password" name="p1" id="p1" onblur="return validateP(this.id)" /></p><p id="pass_pattern" style="font-size:13px">    ( * Must contain min of 8 characters with 1 uppercaseLetter,1 number,1 special char ) </p>
...
elseif(!preg_match("/^(?=.*\d)(?=.*[A-Z])(?=.*[!@#$%^&*()?])[0-9A-Za-z!@#$%^&*()?]{8,50}$/", $password1))
{
echo "<style>#pwd_error{display:block;}</style>";
echo "<style>#pwd_success{display:none;}</style>";
echo "<script>    
...
document.getElementById('GP').setAttribute('class','tablinks');
document.getElementById('General').style.display='none';
document.getElementById('pwd_error').innerHTML='Password must contain minm of 8 characters with atleast an uppercase alphabet, a number and a special character';
...
add_filter('password_change_email', 'send_pwd_change_mail',10, 3);
$user_now=wp_get_current_user();

function send_pwd_change_mail()
{
...
<p>Hi, $displayname</p>
<br />
<p>Your Password is changed at QezyPlay. </p>
<p>Your New Password: $user_pass </p>
...
add_filter('email_change_email', 'send_email_change_mail',10, 3);

$user_now=wp_get_current_user();

function send_email_change_mail()
{
$displayname=$user_now->display_name;
$login_link=site_url()."/login/";
$user_mail=$user_now->user_email;
...
<p>Hi, $displayname</p>
<br />
<p>Your Email is changed at QezyPlay. </p>
