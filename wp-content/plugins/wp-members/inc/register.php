...
function wpmem_registration( $toggle ) {
	...
	case "register":
		...
		if ( is_multisite() ) {
			...
			// Validate username and email fields. //Password Added
			$wpmem_themsg = ( ! $fields['username'] ) ? __( 'Username is a required field', 'wp-members' ) : $wpmem_themsg;
			...
			//$wpmem_themsg = ( username_exists( $fields['username'] ) ) ? "user" : $wpmem_themsg;
				...
				{$wpmem_themsg="Username already exists! Choose different one";}
			...
			//$wpmem_themsg = ( email_exists( $fields['user_email'] ) ) ? "email" : $wpmem_themsg;
					...
					{$wpmem_themsg="Email already exists! Choose different one";}
					if ( $wpmem_themsg ) {
		...
		/* $wpmem_themsg = (! validate_pass( $fields['password']) ) ? __( 'Password must be minm of 8 characters with atleast a no. a special char and an uppercase letter', 'wp-members' ) : $wpmem_themsg;  */
 //added
		...
		jQuery(function(){
			jQuery.jAlert({
				'title': 'Success',
				'content': 'Registration Successfull! Please check your email',
				'closeOnEsc': false,
				'closeOnClick': false
			});});
	</script>";
