...
$defaults = array(
		...
		'message'        => __( 'You entered an invalid username or password.', 'wp-members' ),
		'p_after'        => '</p>',
		'link'           => '<a href="' . $_SERVER['REQUEST_URI'] . '">' . __( 'Click here to continue.', 'wp-members' ) . '</a><style>#wpmem_login,#login_sep{display:none;}</style>',
...
/*$str='<script>alert("Err");$(window).load(function(){document.getElementById("log").setCustomValidity("Invalid username");});</script>';*/
	
echo '<script>
		swal({   title: "Login Error!",   text: "Invalid Username or Password",   type: "error",   confirmButtonText: "Ok" });
...
function wpmem_inc_regmessage( $toggle, $msg = '' ) {
	...
	$defaults = array(
		...
		'div_after'  => '</div>', 
		'p_before'   => '<p id="err_reg" style="text-align: center !important;margin-bottom: -25px !important;color: red !important;">',
		'p_after'    => '</p>',
		'toggles'    => array(
			'user',
			'email',
...
//$str = $args['p_before'] . stripslashes( $args['msg'] ) . $args['p_after'];

if($toggle=="pwdresetsuccess")
{
$str="<script>swal({   title: 'Password Successfully Reset',   text: '".stripslashes( $args['msg'] )."',   type: 'warning',   showCancelButton: false,   confirmButtonColor: '#DD6B55',   confirmButtonText: 'Ok',   cancelButtonText: 'Go HOME',   closeOnConfirm: false,   closeOnCancel: false }, function(isConfirm){   if (isConfirm) {window.location='".home_url()."/';   } else {   window.location='".home_url()."';     } });
</script>";
/*$str= "<script>
swal({   title: 'Password Successfully Reset',   text: '',   type: 'success',   confirmButtonText: 'Ok' });
		</script>";*/
}
else{
$str= "<script>
swal({   title: 'Error!',   text: '".stripslashes( $args['msg'] )."',   type: 'error',   confirmButtonText: 'Ok' });
		</script>";
}
//$str="<p style='text-align: center !important;margin-bottom: 5px !important;'>UserName or E-mail doesnt exists. Please check.</p>";
