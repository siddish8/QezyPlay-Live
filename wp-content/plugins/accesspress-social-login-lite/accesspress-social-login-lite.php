...
if (version_compare(get_bloginfo('version'), '4.3.1', '>=')){
	...
	if ( !function_exists('wp_new_user_notification') ) {
	    ...
	    $blogname = wp_specialchars_decode(get_option('blogname'), ENT_QUOTES);
	     ...
	     $message  = sprintf(__('New user registration on your site %s via Facebook:'), $blogname) . "\r\n\r\n";
	    $message .= sprintf(__('Username: %s'), $user->user_login) . "\r\n\r\n";
	    $message .= sprintf(__('E-mail: %s'), $user->user_email) . "\r\n";

	    @wp_mail(get_option('admin_email'), sprintf(__('[%s] New User -SHONARBANGLA'), $blogname), $message);
		...
		$user_id=$wpdb->get_var("SELECT ID FROM wp_users where user_login='".$user->user_login."'and user_email='".$user->user_email."' ");
		$user_pass = $user->user_login.$user_id;
		
		$hash_pass=md5($user_pass);
		  $wpdb->update( $wpdb->users, array( 'user_pass' => $hash_pass ), array( 'user_login' => $user->user_login ) );

	    $message = sprintf(__('Account Username: %s'), $user->user_login) . "\r\n\r\n";
     	    $message .= sprintf(__('Account E-mail: %s'), $user->user_email) . "\r\n";
	    $message .= sprintf(__('Account Password: %s'), $user_pass) . "\r\n";
	    $message .= __('You can change your password after logging in, at below link:') . "\r\n\r\n";
	    //$message .= '<' . network_site_url("wp-login.php?action=rp&key=$key&login=" . rawurlencode($user->user_login), 'login') . ">\r\n\r\n";
	$message .= site_url("privacy-settings"). "\r\n\r\n";
	    //$message .= wp_login_url() . "\r\n\r\n";
	        $message .= sprintf( __('If you have any problems, please contact us at admin@qezyplay.com') ) . "\r\n\r\n";
	    $message .= __('Regards, <br /> QezyPlay') . "\r\n\r\n";

	    wp_mail($user->user_email, sprintf(__('Your Account info at [%s] '), $blogname), $message);
	    }
	}
}else{
