...
if(isset($_POST['login'])){
	...
	if($match==0){		
		...
		$sql2 = "SELECT id,authkey,agentname FROM agent_info WHERE email = ?";		
		try {
		...
		if($email === $email && $authkey === $authkey_db){
			...
			//Redirect the user to the next page
			//header('LOCATION: agent-home.php');exit;
			header('LOCATION: agent-request-for-subscribe.php');exit;
		}
    ...
    <body>
        ...
        <!--<div class="logo">
        	...
        	<span class="l l5"></span>
		     </div>-->        <center>AgentPortal</center>
      </h1> </header>
                    <div class="auth-content">
                        <p class="text-xs-center">LOGIN TO CONTINUE</p>
                        <form id="login-form" action="" method="POST" novalidate="">
                            <div class="form-group"> <label for="username">Agent Name</label> <input type="text" class="form-control underlined" name="email" id="email" placeholder="Your Name" required onkeypress="return event.charCode !=32" > </div>
